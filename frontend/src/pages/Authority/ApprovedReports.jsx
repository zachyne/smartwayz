...
import { Eye, CheckCircle, MapPin, Calendar, User, Clock } from "lucide-react";
...
const ApprovedReports = () => {
  ...
  const mapReportForUI = (r) => {
    const displayStatus = r.status_name || "Approved";
    ...
    return {
      ...
      statusCode: r.status_name?.toLowerCase().replace(" ", "_") || "approved",
  ...
  const handleAction = async (reportId, action) => {
    try {
      if (action === "start") {
        await reportAPI.updateStatus(reportId, 3); // 3 is IN_PROGRESS status
        await fetchReports();
        setSelectedReport(null);
      }
    } catch (err) {
      console.error("Failed to update status:", err);
      alert("Failed to start work on this report");
    }
  };
  ...
  const fetchReports = async () => {
    try {
      ...
      const approvedReports = raw.filter(r =>
        r.status_name?.toLowerCase() === "approved"
      );

      const mapped = approvedReports.map(mapReportForUI);
      setReports(mapped);
    } catch (err) {
    ...
    <div className="flex-1 text-white font-[Kanit] bg-gradient-to-b from-[#37366B] to-[#0A0E27] min-h-screen pt-20 lg:pt-0">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="bg-green-500 text-white text-sm sm:text-lg font-bold px-3 sm:px-4 py-1 sm:py-2 rounded-full">
                {filteredReports.length}
              </span>
            </div>
            <p className="text-sm sm:text-base lg:text-lg text-gray-300">Reports ready to be worked on</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-6 md:p-8">
        ...
        <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            <p className="text-green-200 font-semibold">Successfully Approved</p>
            <p className="text-green-300/80 text-sm">{filteredReports.length} reports have been approved and are ready for action.</p>
          </div>
        </div>
        ...
        {!isLoading && filteredReports.length > 0 && (
          <div className="space-y-4">
            {filteredReports.map((report) => (
              <div
                key={report.id}
                className="bg-[#1E1C3A]/60 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-gray-700/50 hover:border-green-500/50 transition-all"
              >
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <button
                      onClick={() => handleAction(report.id, 'start')}
        ...
        {!isLoading && filteredReports.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-400">No approved reports found</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-[#1E1C3A] rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden border border-gray-700 flex flex-col">
            ...
            <div className="sticky top-0 bg-[#1E1C3A] border-b border-gray-700 p-4 sm:p-6 md:p-8">
              ...
              <div className="pt-4 border-t border-gray-700">
                <button
                  onClick={() => handleAction(selectedReport.id, 'start')}
                  ...
                  {selectedReport.imageUrls?.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {selectedReport.imageUrls.map((url, idx) => (
                        <div key={idx} className="aspect-video bg-gradient-to-br from-gray-700 to-gray-800 rounded-lg overflow-hidden border border-gray-600 hover:border-green-500 transition-all cursor-pointer">
                          <img
                            src={url}
                            alt={`Evidence ${idx + 1}`}
