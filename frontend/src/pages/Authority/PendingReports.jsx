...
import { Search, Filter, Eye, Check, X, MapPin, Calendar, User, AlertCircle } from "lucide-react";
...
const PendingReports = () => {
  ...
  const handleAction = async (reportId, action) => {
    try {
      ...
      await reportAPI.updateStatus(reportId, statusId);
  ...
  const fetchReports = async () => {
    try {
      ...
      const pendingReports = raw.filter(r =>
        r.status_name?.toLowerCase() === "pending"
      );

      const mapped = pendingReports.map(mapReportForUI);
      setReports(mapped);
    } catch (err) {
    ...
    <div className="flex-1 text-white font-[Kanit] bg-gradient-to-b from-[#37366B] to-[#0A0E27] min-h-screen pt-20 lg:pt-0">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="bg-orange-500 text-white text-sm sm:text-lg font-bold px-3 sm:px-4 py-1 sm:py-2 rounded-full">
                {filteredReports.length}
              </span>
            </div>
            <p className="text-sm sm:text-base lg:text-lg text-gray-300">Reports awaiting your review and approval</p>
          </div>
          <button
            onClick={() => setShowFilters(!showFilters)}
            className="sm:hidden bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium transition-all"
          >
            <Filter size={18} />
            Filters
          </button>
      ...
      <div className="p-4 sm:p-6 md:p-8">
        ...
        <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            <p className="text-orange-200 font-semibold">Action Required</p>
            <p className="text-orange-300/80 text-sm">You have {filteredReports.length} reports pending review.</p>
          </div>
        </div>
        ...
        <ReportFilters
          ...
          accentColor="orange" // Change color based on page
        />
        ...
        {!isLoading && filteredReports.length > 0 && (
          <div className="space-y-4">
            {filteredReports.map((report) => (
              <div
                key={report.id}
                className="bg-[#1E1C3A]/60 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-gray-700/50 hover:border-orange-500/50 transition-all"
              >
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <button
                      ...
                      className="flex-1 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-medium transition-all"
                    >
                      <Eye size={16} />
                      View Details
                    </button>
                    <button
                      ...
                      className="flex-1 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-medium transition-all"
                    >
                      <Check size={16} />
                      Approve
                    </button>
                    <button
                      ...
                      className="flex-1 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-medium transition-all"
                    >
                      <X size={16} />
                      Reject
                    </button>
        ...
        {!isLoading && filteredReports.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-400">No pending reports found</p>
          </div>
        )}
                  ...
                  {selectedReport.images > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {Array.from({ length: selectedReport.images }).map((_, idx) => (
                        <div key={idx} className="aspect-video bg-gradient-to-br from-gray-700 to-gray-800 rounded-lg overflow-hidden border border-gray-600 hover:border-orange-500 transition-all cursor-pointer">
                          <img
                            src={`https://images.unsplash.com/photo-${
                              ...
                              selectedReport.id === 4 ? "1449824913935-59a10b8d2000" :
